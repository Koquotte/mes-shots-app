...
// --- NOUVEAUTÉ : GESTION DES FAVORIS ---

// Récupère les favoris depuis la mémoire du navigateur
function getFavorites() {
    const favorites = localStorage.getItem('favoriteShots');
    return favorites ? JSON.parse(favorites) : [];
}

// Sauvegarde les favoris dans la mémoire du navigateur
...
function toggleFavorite(recipeName, event, recipeIndex = -1) {
    if (event) {
        event.stopPropagation(); // Empêche d'ouvrir la recette quand on clique sur l'étoile
    }
    ...
    if (favoriteIndex > -1) {
        favorites.splice(favoriteIndex, 1); // Retire le favori
    } else {
        favorites.push(recipeName); // Ajoute le favori
    }
    
    saveFavorites(favorites);

    // Rafraîchit la vue pour montrer le changement
    if (recipeIndex > -1) {
        renderRecipeDetail(recipeIndex);
    } else {
        renderRecipeList();
    }
}

// --- NOUVEAUTÉ : GESTION DES FILTRES ---

// Met à jour le filtre actif et rafraîchit la liste
...
// --- VOS DONNÉES DE RECETTES (AVEC CORRECTIONS) ---
const recipesData = [
    {
        name: "B-52",
        ingredients: [
    ...
    },
    {
        ...
        ingredients: [
            ...
            { name: "Tranche de citron vert", baseQuantity: 1.0, unit: "" } // Unité corrigée
        ]
    },
    {
        name: "Orgasme",
        ingredients: [
            { name: "Baileys", baseQuantity: 2.0, unit:"cl"},
    ...
    },
    {
        ...
        ingredients: [
            { name: "Whiskey Canadien", baseQuantity: 3.0, unit:"cl"},
    ...
    },
    {
        ...
        ingredients: [
            ...
            { name: "Jus d'ananas", baseQuantity: 3.0, unit:"cl"}, // Typo corrigée
...
// --- FONCTION D'AFFICHAGE DE LA LISTE (MIS À JOUR) ---
function renderRecipeList() {
    ...
    if (activeFilter) {
        if (activeFilter === 'FAVORITES') {
            ...
            filteredRecipes = recipesData.filter(recipe => 
                recipe.ingredients.some(ing => ing.name === activeFilter)
            );
        }
    }
    ...
    allIngredients.sort().forEach(ingredient => {
        html += `<button class="filter-btn ${activeFilter === ingredient ? 'active' : ''}" onclick="filterBy('${ingredient}')">${ingredient}</button>`;
    });
    html += `</div>`;

    // NOUVEAU : Liste des recettes avec étoile
        ...
        filteredRecipes.forEach(recipe => {
            ...
            html += `
                <div class="recipe-card" onclick="renderRecipeDetail(${index})">
                    ${recipe.name}
                    <div class="favorite-star ${isFavorite ? 'active' : ''}" onclick="toggleFavorite('${recipe.name}', event)">★</div>
                </div>
            `;
        });
    }
...
// --- FONCTION D'AFFICHAGE DÉTAILLÉ (MIS À JOUR) ---
        ...
        return recipe.ingredients.map(ing => {
            ...
            return `
                <div class="ingredient">
                    ...
                    <span class="ingredient-quantity">${adjustedQuantity.toFixed(1).replace('.0', '')} ${ing.unit}</span>
                </div>
            `;
        }).join('');
    }
    ...
    let detailHtml = `
        <div class="detail-view">
            ...
            <div class="header-container">
                ...
                <div class="favorite-star-detail ${isFavorite ? 'active' : ''}" onclick="toggleFavorite('${recipe.name}', event, ${recipeIndex})">★</div>
            </div>
